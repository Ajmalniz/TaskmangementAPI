...
Complete guide to handling errors in FastAPI applications with proper HTTP status codes, HTTPException, and agent-friendly error responses.
...
* **422**: "Your JSON doesn't match my schema" (Pydantic catches this automatically)
* **400**: "Your data matches the schema, but violates my business rules" (You handle this manually)

**Examples:**

```python
# 422 - Schema validation fails
POST /tasks
{"description": "no title"}  # Missing required "title" field
→ 422 Unprocessable Entity

# 400 - Business rule fails
POST /tasks
{"title": "   "}  # Title is present, but empty whitespace
→ 400 Bad Request (you check this in your code)

# 422 - Type validation fails
POST /tasks
{"title": 123}  # Title should be string
→ 422 Unprocessable Entity

# 400 - Business logic fails
PUT /tasks/1
{"title": "Valid", "status": "invalid_status"}  # Invalid status value
→ 400 Bad Request (you validate status in your code)
```

## Complete Error Handling Example

Here's a complete Task API with proper error handling:
...
# Expected: 400 with "Invalid status. Must be one of: pending, in_progress, completed"
```
...
**Mistake 4**: Not using status module constants
...
```python
# Unhelpful - what's wrong?
raise HTTPException(status_code=400, detail="Invalid input")

# Helpful - specific and actionable
raise HTTPException(
    status_code=status.HTTP_400_BAD_REQUEST,
    detail=f"Invalid status '{status}'. Must be one of: pending, in_progress, completed"
)
```
...
- Use HTTPException for HTTP errors (not Python exceptions)
- Always raise HTTPException (don't just create it)
- Use status module constants for self-documenting code
- Return appropriate status codes (404 for not found, 400 for bad request)
- Distinguish 422 (schema validation) from 400 (business rules)
- Design error messages for both humans and agents
- Include context in error messages (what's valid, what was wrong)
- Never expose internal implementation details
- Consider structured error responses for agent-friendly APIs
- Agents check status codes first, then parse error bodies
